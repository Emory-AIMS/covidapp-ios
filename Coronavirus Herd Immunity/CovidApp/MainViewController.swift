...
//        scrollView.contentSize = CGSize(width: view.bounds.width, height: 1175)
    ...
    private func updateStatus(){
        ...
        if let data = PushNotificationData.readNotificationDate(){
            ...
            if let status = data.status{
                ...
                var titleColor = UIColor(red: 137 / 255, green: 34 / 255, blue: 53 / 255, alpha: 1)
                ...
                case 4:
                    ...
                    msg = NSLocalizedString("Quarantine Status message low ris", comment: "Quarantine Status message low ris")
                    titleColor = .white
                    break
                case 5:
                    print("mid risk")
                    text = NSLocalizedString("Mid risk", comment: "Mid risk")
                    msg = NSLocalizedString("Quarantine Status message mid risk", comment: "Quarantine Status message mid risk")
                    titleColor = .white
                    break
                case 6:
                    print("high risk")
                    text = NSLocalizedString("High risk", comment: "High risk")
                    msg = NSLocalizedString("Quarantine Status message high risk", comment: "Quarantine Status message high risk")
                    titleColor = .white
                    break
                default:
    ...
    @IBAction func showID(_ sender: Any) {
        if let did = StorageManager.shared.getIdentifierDevice() {
            ...
            let storyBoard : UIStoryboard = UIStoryboard(name: "Main", bundle: nil)
            let vc = storyBoard.instantiateViewController(withIdentifier: "IdentificationCodeViewController") as! IdentificationCodeViewController
            vc.modalPresentationStyle = .overCurrentContext
            let codeToShow = did.description + didChecksum.description
            vc.identificationCode = "\(codeToShow.addingDashes())"
